...
const buildEndpoint = (period?: string, dateFrom?: string, dateTo?: string) => {
  ...
  if (!url || !key) {
    return {
      endpoint: null,
      key: null,
      error: "Configure NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY.",
    };
  }
...
export function useElapsedTimes(params: UseElapsedTimesParams = {}): UseElapsedTimesResult {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development") {
      console.debug("[useElapsedTimes] fetch:start", { requestKey, correlationId, endpoint, timeoutMs: REQUEST_TIMEOUT_MS });
    }
    ...
    const fetchTimes = async () => {
      ...
      try {
        ...
        if (typeof cachedLatestMs === "number" && refreshToken > 0) {
          ...
          if (latestResponse.ok) {
            ...
            if (!Number.isNaN(latestMs) && latestMs === cachedLatestMs) {
              ...
              if (process.env.NODE_ENV === "development") {
                console.debug("[useElapsedTimes] fast-check:no-change", { requestKey, cachedLatestMs, latestMs });
              }
              return;
            }
            if (process.env.NODE_ENV === "development") {
              console.debug("[useElapsedTimes] fast-check:changed", { requestKey, cachedLatestMs, latestMs });
            }
        ...
        if (process.env.NODE_ENV === "development") {
          console.debug("[useElapsedTimes] fetch:pages", { requestKey, total: data.length });
        }
        setTimes(data);
        ...
        if (abortLike) {
          if (process.env.NODE_ENV === "development") {
            ...
            console.debug("[useElapsedTimes] fetch:aborted", { requestKey, correlationId, elapsed, reason });
          }
          return;
        }
        ...
        if (process.env.NODE_ENV === "development") {
          console.debug("[useElapsedTimes] fetch:end", { requestKey, correlationId, elapsed, aborted: controller.signal.aborted, timeoutFired });
        }
      }
    };
