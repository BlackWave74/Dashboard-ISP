...
export default function Gamificacao() {
  ...
  const ranking = useMemo<ConsultantScore[]>(() => {
    ...
    return Array.from(byConsultant.entries())
      .map(([name, s]) => {
        ...
        const streak = Math.min(s.done, 10); // simplified
        const badges: string[] = [];
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute top-[10%] left-[20%] h-[500px] w-[500px] rounded-full opacity-15 blur-[140px]" style={{ background: "radial-gradient(circle, hsl(45 90% 55%), transparent 70%)" }} />
      <div className="pointer-events-none absolute bottom-[20%] right-[10%] h-[400px] w-[400px] rounded-full opacity-10 blur-[120px]" style={{ background: "radial-gradient(circle, hsl(280 70% 55%), transparent 70%)" }} />
            ...
            {[1, 0, 2].map((podiumIdx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Podium bar */}
                  <motion.div
                    className={`${heights[podiumIdx]} w-28 mt-3 rounded-t-xl flex flex-col items-center justify-start pt-4 border border-border/30`}
                    style={{
                      background: isFirst
                        ? "linear-gradient(180deg, hsl(45 90% 55% / 0.2), hsl(45 90% 55% / 0.05))"
                        : "linear-gradient(180deg, hsl(234 89% 64% / 0.15), hsl(234 89% 64% / 0.03))",
                    }}
                    initial={{ height: 0 }}
                    animate={{ height: "auto" }}
                  ...
                  >
                    {medals[podiumIdx]}
                    <span className="text-2xl font-black text-foreground/30 mt-2">#{podiumIdx + 1}</span>
                  </motion.div>
                </motion.div>
              );
            })}
            ...
            <CardContent className="p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {Object.entries(BADGE_DEFS).map(([key, def]) => (
                  <div key={key} className="flex items-center gap-2 rounded-xl bg-card/50 border border-border/20 p-3">
                ...
                {ranking.map((person, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="flex items-center gap-4 rounded-xl bg-card/30 border border-border/20 p-3 hover:bg-card/60 transition-all"
                    >
                      {/* Position */}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className="text-[9px] border-border/30" style={{ color: levelColor, borderColor: `${levelColor}40` }}>
                            {levelName}
                          </Badge>
                        </div>
