...
const STATUS_COLORS: Record<string, { dot: string; bg: string; text: string }> = {
  overdue: { dot: "bg-red-500", bg: "bg-red-500/10 border-red-500/20", text: "text-red-400" },
  pending: { dot: "bg-amber-400", bg: "bg-amber-500/10 border-amber-500/20", text: "text-amber-400" },
  done: { dot: "bg-emerald-500", bg: "bg-emerald-500/10 border-emerald-500/20", text: "text-emerald-400" },
  unknown: { dot: "bg-muted-foreground", bg: "bg-muted/30 border-border/30", text: "text-muted-foreground" },
};
...
export default function Calendario() {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute top-[40%] right-[-10%] h-[500px] w-[500px] rounded-full opacity-15 blur-[140px]" style={{ background: "radial-gradient(circle, hsl(234 89% 50%), transparent 70%)" }} />
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1400px] space-y-6 px-6 pt-6 md:px-10 pb-16">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
                  ...
                  {days.map((day, i) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`relative flex flex-col items-center justify-start rounded-xl p-1.5 min-h-[72px] transition-all border ${
                          isSelected
                            ? "bg-primary/15 border-primary/40 shadow-lg shadow-primary/10"
                            : isToday
                            ? "bg-primary/5 border-primary/20"
                            : "bg-card/20 border-transparent hover:bg-card/50 hover:border-border/30"
                        }`}
                      >
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            <Card className="bg-card/40 border-border/30 backdrop-blur-xl sticky top-6">
              <CardContent className="p-5">
                      ...
                      {selectedTasks.map((t, i) => {
                        ...
                        return (
                          <motion.div
                            ...
                            className={`rounded-xl border p-3 ${colors.bg}`}
                          >
