...
export function TaskListTable({ tasks }: TaskListTableProps) {
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-[hsl(var(--task-border))] bg-[hsl(var(--task-surface))]">
      {/* Header */}
      <div className="hidden md:grid grid-cols-[minmax(140px,1.5fr)_90px_90px_160px_160px_100px] bg-[hsl(var(--task-bg))] border-b border-[hsl(var(--task-border))]">
          ...
          return (
            <div key={key}>
              <motion.div
                ...
                className={`group grid grid-cols-1 md:grid-cols-[minmax(140px,1.5fr)_90px_90px_160px_160px_100px] bg-transparent transition-colors cursor-pointer hover:bg-[hsl(var(--task-surface-hover))] ${isOverdue ? "task-shake" : ""}`}
              >
                {/* Task name */}
